...
- **Status:** [~] in_progress — Project/Task models + endpoints (full security contract + optimistic version, **integration-tested**), keyboard-accessible kanban, projects list screen, **and live `ServerpodTaskRepository` + `ServerpodProjectRepository`** through the authenticated generated client (task read proven end-to-end via `tool/auth_smoke.dart`; project read-path proven by `feature_reads_test.dart`; toggle with `--dart-define=MAKERFLOW_LIVE=true`). **Task create write-path wired** end-to-end: `TaskRepository.create` (in-memory + live `client.task.create`) behind an accessible "New task" dialog (labelled fields, required-field validation, busy state, typed-error surface + live-region announce); widget-tested (validation + create + board refresh). Remaining: task list/calendar views + edit/update write-path from the UI
- **Agent Persona:** serverpod-backend + flutter-ui
- **Priority:** P0
- **Complexity:** XL
