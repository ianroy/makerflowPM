...
- [x] Filter builder w/ per-type operators; persisted per view (saved views; built-in tabs keep session-only config — see decisions); announced result counts
- [x] Multi-sort (the Sort dialog); column-header click cycles asc → desc → off with ▲/▼ markers
- [x] Group-by-any-field incl. custom dropdowns/labels/checkbox/person + assignee/project; per-group counts (fuller aggregations = fl-8-column-summaries)
- [x] Tests: filter narrows (widget), sort orders (unit + click-cycle widget), group-by-person groups correctly (widget + unit)

**Agent Decisions:**
- `2026-07-16` — **Shipped.** The pure core lives in `data/view_config.dart`: `ViewConfig {join, groups[], sorts[], groupBy}` with a tolerant JSON codec stored in `CustomView.filtersJson` (legacy `'{}'` decodes to the default), a **field registry** (`buildFieldDescriptors`: built-ins title/status/priority/due/project/assignee + one descriptor per custom field, ids matching column keys) with per-kind operator sets (text contains/is/empty…, number comparators incl. tolerant string-parse, date is/before/after on date-only strings, option is/isNot, multi has/notHas, checkbox isTrue/isFalse), and the evaluator: `applyFilter` (rows AND/OR within a group, groups AND/OR at the top — the spec's one level of nesting), `applySort` (multi-level, option-kind sorts by option order so priority sorts low→urgent not alphabetically, **empties last in both directions** — caught by a unit test when the desc flip inverted them), `computeGroups` (status keeps today's exact six-group behavior with bare keys so collapse state + every pre-existing test survives; option fields render ALL options as groups + "No X"; dynamic fields render present values; checkbox → checked/unchecked; label colors carried, others cycle the group palette). 9 unit tests pin the contract.
- `2026-07-16` — **UI.** Toolbar Sort/Group-by stubs became real controls (+ a Filter control) with active-state labels (`Filter (2)`, `Sort (1)`, `Grouped`) and AT labels; the filter builder is draft-edit + Apply (typing must not refilter live) with per-kind value editors (option dropdowns, date picker, validated number field); Sort dialog = the multi-level editor; header CLICKS cycle a single-column sort asc → desc → off with `▲/▼` (+ level number when multi) — header click replaces the sort list; the dialog owns multi-sort. Every apply announces the fresh matched count (WCAG 4.1.3). The Main Table's group rendering went generic (`TaskGroup {key,label,color,tasks}` computed in kanban_screen): grouped add-item rows carry the group's field value into the new task (label/select/number/checkbox/priority/project; assignee groups create plain to-do until an assignee write path exists) — **gotcha fixed en route:** group keys are `<fieldId>:<value>` and custom-field ids are themselves `cf:<key>`, so the value separator is the SECOND colon for them (the first-colon split wrote `material:Wood` as field `cf`). Saved views round-trip the whole config through `filtersJson` (select applies it; dirty-state + Save/Save-as/Reset + rename/share/duplicate all carry it; leaving to a built-in tab resets to the session default).
- `2026-07-16` — **Deviations + deferrals:** (1) *Built-in quick views keep session-only config* — "persisted per view" holds for saved views; persisting the default table's filters into `__table_layout.filtersJson` wants the same derive seam and is cheap to add when someone asks. (2) *Person quick-filter chip stays a stub* — still blocked on the member-directory endpoint (same as the person picker); assignee IS filterable/groupable through the builder meanwhile. (3) *URL `?view=` param deferred again* — it was queued to ride this card but adds router-state plumbing orthogonal to filtering; now tracked as its own follow-up rather than a rider. (4) Per-column menu (sort/filter from the header) folded into the header-click affordance for now. Verified: app **62/62** (9 unit + 5 widget new in `view_config_test.dart`/`filter_sort_group_test.dart`), analyze clean, live web bundle rebuilt, and the filter path smoke-tested in the running demo (condition built in the dialog → board narrowed to the matching row → badge `Filter (1)` → cleared). Server untouched (client-side evaluation per spec; push-down lands with pagination).
...
- [x] Aggregation picker + per-group/board footers + persistence + text equivalents

**Agent Decisions:**
- `2026-07-16` — **Shipped (S, zero new persistence plumbing).** The summary choice is a nullable `summary` field ON `ColumnPref` — it rides the existing columns codec, so it persists everywhere columns already do (the `__table_layout` default via the debounced save, saved views via their dirty-state Save flows, org resets) for free. Compute lives beside the evaluator in `view_config.dart`: `summaryOpsFor(kind, fieldId)` (number sum/avg/min/max/count · date range · checkbox percent · option/multi unique/count · status battery/count) + `computeSummary` returning a text equivalent always and a status mix for the battery; ops invalid for a kind return null, so a stale pref after a field type-change renders as empty instead of lying. Rendering: a **board grand-total footer always renders** (its muted `+` cells are the discoverable entry point — per-group-only footers would be unreachable when nothing is selected yet); **per-group footers appear once any column has a summary**, computed over that group's (filtered) tasks; the status battery renders as a compact stacked bar with the counts in the cell's Semantics label (WCAG 1.1.1). Picker paths: tapping any footer cell opens the type-valid op menu, and the **Columns popover carries the same picker per column** (Σ icon, brand-tinted when active) as the keyboard/AT path. Tests: +4 unit (op math incl. battery mix/percent/range + stale-op null) and +3 widget (grand→group footer flow + `__table_layout` persistence round-trip, battery AT label with exact numbers, the popover path) → app **69/69**, analyze clean, live bundle rebuilt. Persistence path identical to the twice-live-proven column pipeline, so no separate browser smoke this round. Deferred: person-column `unique` works but shows id-based values until the member directory lands; richer aggregations (median, weighted) with fl-8-dashboards.
...
>
> **Position:** plan-M1 interactive core done (NS-M1.1–M1.4); the monday-style redesign is live through **UI-3a** (Vibe design system → shell → board chrome → Main Table as default view); **Phase 8 is rolling** — `fl-8-view-field-endpoints`, `fl-8-column-registry`, and `fl-8-custom-fields` shipped (customization endpoints; theme/sidebar persistence; the persisted column system; and 9 tier-1 custom field types end-to-end with D6 values, server validation, coercion, per-type editors, and the field manager). Everything green (2026-07-16: server **36/36** over live Postgres, app **48/48**, design 7/7, root CI green). `fl-8-saved-views` shipped too, and the whole rebuild is **merged to `main`** ([PR #6](https://github.com/ianroy/makerflowPM/pull/6), 2026-07-16, 42 commits) — the DO demo spec keeps deploying from `staging`; production cutover = flip `branch: main`.
>
> **Next, in order:**
> 1. **Phase 8 continues (5/11 done)** — `fl-8-column-summaries` (footer aggregations + per-group batteries) → `fl-8-subitems` → drag suite → templates → dashboards → automations.
...
- `2026-07-16` — `fl-8-column-summaries` — **Footer aggregations shipped.** `ColumnPref` gained a nullable `summary` op that rides the existing columns codec (persistence everywhere columns persist — zero new plumbing); `computeSummary` in `view_config.dart` (number sum/avg/min/max/count, date range, checkbox %, unique, status battery w/ text equivalent; stale ops → null, never wrong); the board grand-total footer always renders as the discoverable entry point, per-group footers appear once a summary exists, and the picker lives on both footer-cell tap and the Columns popover (Σ, the AT path). +7 tests → app **69/69**. Phase 8: **6/11**. Next: `fl-8-subitems`.
- `2026-07-16` — `fl-8-filter-sort-group` — **Filters, multi-sort, and group-by-any-field shipped.** Pure core in `data/view_config.dart` (model + tolerant `filtersJson` codec + field registry + evaluator; 9 unit tests) drives the toolbar's new Filter/Sort/Group-by controls: an Airtable-style builder (AND/OR groups, per-kind operators + value editors, Apply/Clear, announced result counts), a multi-level Sort dialog + header clicks cycling asc→desc→off with ▲/▼ markers, and generic table groups (all option values + "No X", label colors carried, grouped ghost rows write the group's field value into new tasks). Saved views round-trip the whole config; built-in tabs keep session-only config. Two bugs caught en route: desc sort inverted the empties-last rule (unit test), and `cf:`-prefixed field ids broke the group-key split (widget test). Deferred: URL `?view=` (now its own follow-up), Person quick chip (member directory), default-table filter persistence. App **62/62** · analyze clean · live bundle rebuilt · filter path smoke-tested in the demo. Phase 8: **5/11 cards done**. Next: `fl-8-column-summaries`.
- `2026-07-16` — `fl-merge-to-main` — **The rebuild is on `main`.** Owner merged [PR #6](https://github.com/ianroy/makerflowPM/pull/6) (42 commits, 2026-06-15 → 2026-07-16: backend contract + monday-style UI + Phase-8 customization platform + CI + the full-demo DO spec + doc refresh; all checks green on the tip). Duplicate PR #5 closed as superseded. Post-merge state: `staging` synced with the merge commit; the DO demo spec deliberately keeps deploying from `staging` so the demo tracks the latest work — production cutover = flip `branch: main` in `.do/app.yaml`. Owner-blocked items now down to two: the doctl token (live deploy) and the human AT pass (R1).
