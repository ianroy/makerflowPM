...
> **fl-8-column-summaries (2026-07-16):** footer aggregations live. The summary op is a nullable field ON `ColumnPref` — persistence rides the columns codec everywhere (default layout, saved views, dirty flows) with zero new plumbing. Type-valid ops per column (number sum/avg/min/max/count · date range · checkbox % · unique · status **battery** w/ text-equivalent Semantics); the board grand-total footer always renders (its muted + cells are the entry point), per-group footers appear once any summary exists (computed over the filtered group); pickers on footer-cell tap AND per-column in the Columns popover (Σ, the AT path); stale ops render empty, never wrong. +7 tests → app **69/69** · analyze clean · live bundle rebuilt. Phase 8 now **6/11**.
>
> **fl-8-filter-sort-group (2026-07-16):** the toolbar's Filter / Sort / Group-by are real. Pure core in `data/view_config.dart` — `ViewConfig` (filter groups + sorts + groupBy) with a tolerant codec in `CustomView.filtersJson`, a field registry spanning built-ins (title/status/priority/due/project/assignee) + every custom field, per-kind operator sets, and the client-side evaluator (`applyFilter` w/ AND/OR at both levels, `applySort` multi-level w/ option-order sorting + empties-last both directions, `computeGroups` — status keeps the exact six-group default; option fields group by every option + "No X"; label colors carried). UI: Airtable-style filter builder (draft + Apply, per-kind value editors, announced result counts, active-count badges), Sort dialog + header-click cycling (▲/▼ markers), group-by picker; grouped ghost add-rows write the group's field value into the new task; saved views persist/restore the whole config through their dirty-state flows. Two en-route bugs fixed by tests: desc-sort inverted empties-last; `cf:` ids broke the group-key split. Deferred (tracked): URL `?view=`, Person quick chip, default-table filter persistence. App **62/62** (+14) · analyze clean · live bundle rebuilt · filter path smoke-tested in the demo. Phase 8 now **5/11**.
>
> **fl-8-saved-views (2026-07-16):** view tabs are real **saved views**. `CustomView.viewType` (table|kanban|list|calendar; migration `20260716134002811`); `SavedViewVm` + list/save/delete on the ViewRepository seam (`__table_layout` filtered out); tabs = built-in quick views (the fixed default) + named saved views (shared ones marked, people icon) + **"+"** that captures the current layout + surface as a new view. Selecting applies the view's columns as local prefs and its type as the surface; while active, layout edits go **dirty** (Save / Save as new / Reset chip, announced) and never touch the `__table_layout` default; the active tab's menu offers Rename / Duplicate / Share / Delete (owner-or-admin server gate). Deferred with rationale on the card: URL view id (→ fl-8-filter-sort-group), per-user default marker, calendar surface (→ UI-7). Live-verified: "Wood shop" created in the demo → hard reload → persists from Postgres. Server **36/36** (+viewType round-trip) · app **48/48** (+5 `saved_views_test.dart`) · analyze clean · live bundle rebuilt.
>
> **fl-8-custom-fields (2026-07-15):** custom fields live **end-to-end** — the XL Phase-8 card. Server (commit `9e5c06c`): `Task.customFieldsJson` per **D6** (+migration), `business/custom_fields.dart` validates every task write against the org's FieldConfig definitions (type match, option lists, unknown keys rejected, null clears, typed Conflicts) and owns the **type-change coercion**: `FieldConfigEndpoint.save` warns-before-write with the affected-value count, then converts on `coerceValues` retry (safe matrix; unconvertible values cleared, versions bumped). Client: `TaskVm.customFields` (tolerant decode), whole-bag-replace repo semantics (callers merge), `FieldRepository` + `taskFieldConfigsProvider`, and the **column registry went dynamic** — one spec per definition (`cf:`-prefixed keys so custom names can't collide with built-ins), per-type cells/editors (`custom_field_cells.dart`: in-place checkbox, AA-ink colored label cells, option/multi-select pickers, date picker, validated scalar dialogs), the same editors staged in the task dialog, and a **field manager** in the Columns popover (add/edit/delete + label color grid; server's workspaceAdmin gate is the authority). Live smoke: field created in the demo → value set → hard reload → definition + value persist from Postgres. Adversarial review workflow (4 lenses × 2 refuters) ran over the diff pre-commit. Server **33/33** (+4) · app **43/43** (+7) · analyze clean · live web bundle rebuilt. Follow-up: person picker needs a member-directory endpoint (values render "User #N").
>
> **fl-8-column-registry (2026-07-15):** the Main Table's hardcoded columns became a **column system**. `_TaskColumnSpec` registry (status/due/priority; item-name pinned + flexible, outside prefs) rendered through per-user `ColumnPref {key,width,hidden}[]` overlays — tolerant merge (unknown keys dropped, new columns auto-appended) so layouts survive schema drift both ways. Interactions: **resize** via 6px header-boundary handles (`resizeColumn` cursor, live width, min 90/80/60, max 420), **double-tap autofit** (TextPainter over header + every cell), **reorder** by dragging a header (drop-before, indicator painted with `foregroundDecoration` so headers stay pixel-aligned with cells), **show/hide + keyboard/AT reorder + reset** in the toolbar **Columns** popover (labeled switches, `Move <col> up/down`, hidden-count on the button). Persistence: new `ViewRepository` seam — live impl writes debounced (500ms) to a per-user `CustomView` named `__table_layout` via the fl-8 ViewEndpoint; the table **derives** its columns from local edits + the org-scoped saved layout (both watched — a listen-based hydration failed the live smoke because loads finishing while the table is unmounted never fire a fresh mount's listen; fixed same-day + regression-tested, full story on the card). **Verified in the running demo:** sign-in → drag-resize → page reload → the saved width renders back out of Postgres (`tool/view_layout_smoke.dart` covers the client load path headlessly). Deviations recorded on the card: live-resize instead of a ghost guide-line, popover search deferred to custom fields. Server untouched. App **36/36** (+8 `column_registry_test.dart`) · analyze clean · live web bundle rebuilt.
>
> **fl-8-view-field-endpoints (2026-07-14):** Phase 8 opens — `ViewEndpoint` (saved views: owner-or-shared, version conflicts, soft-delete), `FieldConfigEndpoint` (workspaceAdmin-gated custom-field definitions, server-side type allow-list, unique-key conflicts, immutable keys), `PreferenceEndpoint` (self-service, session-pinned). `CustomView` gained `version`+soft-delete, `UserPreference` gained `uiJson` (new migration). **Theme + sidebar collapse now persist across restarts** (live mode; `prefsLoadProvider` listen-hydration in the shell). Server **29/29** · app **28/28** · analyze clean · demo migrated + rebuilt.
>
